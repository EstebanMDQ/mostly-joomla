...
<script type="text/javascript" charset="UTF-8" src="<?php echo QFORMS_URI_TEMPLATES; ?>cssQuery-p.js"></script>
...
<?php echo $extra_html; ?>
<script type="text/javascript">document.title +=' - <?php echo $title; ?><?php if($subtitle) { ?> - <?php echo $subtitle; ?><?php
...
<div class="title title_<?php echo $this->abm_name.'_'.$this->currAction;?>"><?php echo $title; ?></div>
<?php if($subtitle) { ?><div class="subtitle"><div class=""><?php echo $subtitle; ?></div></div><?php
...
?>
    ...
    <?php if( !empty($data_filters) ) { ?>
		<div class="actions_right actions_right_filters filter_<?php echo $this->abm_name.'_'.$this->currAction;?>">
		<?php
        	$filter_idx = 0;
    ...
    <?php if( !empty($data_orderby) ) { ?>
		<div class="actions_right actions_right_sorting filter_<?php echo $this->abm_name.'_'.$this->currAction;?>">
        	<span class="input_item input_item_sort"><span class="label"><?php echo QForms::trans('Order by'); ?></span>
    		<select name="<?php echo $this->abm_prefix; ?>orderBy"><?php echo QForms::HTML_options($data_orderby, $this->abm_orderBy,true); ?></select>
        	</span>
        	<span class="input_item input_item_rpp"><span class="label"><?php echo QForms::trans('Showing'); ?></span>
...
<div class="actions_right" ><?php $this->evtBeforeData(); ?></div>

		<div class="actions_right filter_buttons_<?php echo $this->abm_name.'_'.$this->currAction;?>">
        <input type="submit" value="<?php echo QForms::trans('Apply'); ?>" class="xfButtonBase button" />
        <input type="submit" value="<?php echo QForms::trans('Clear'); ?>" onclick="for(var i=0 ; i < this.form.elements.length ; i++) { if(this.form.elements[i].selectedIndex) this.form.elements[i].selectedIndex=-1; else if(this.form.elements[i].type!='hidden') this.form.elements[i].value=''; }" class="xfButtonBase button" />
        </div>
...
<?php if($actions_top) { ?>
	<div class="actions_left buttons_top_<?php echo $this->abm_name.'_'.$this->currAction;?>">
		<?php echo $actions_top; ?>
	</div>
<?php } /* if($actions_top) */ ?>
<?php if($actions_bottom) { ?>
	<div class="actions_right buttons_bottom_<?php echo $this->abm_name.'_'.$this->currAction;?>" >
...
<div class="grid_pager">
	...
	<div class="pager">
		<input type="button" title="Ir a primero" class="first" value="" <?php if($url_pages[0]){?> onclick="self.location='<?php echo $url_pages[0]; ?>';"<?php } ?> />
		<input type="button" title="Anterior" class="previous" value="" <?php if($url_pages[0]){?> onclick="self.location='<?php echo $url_pages[1]; ?>';"<?php } ?> />
		...
		<input type="button" title="Siguiente" class="next" value="" <?php if($url_pages[3]){?> onclick="self.location='<?php echo $url_pages[3]; ?>';"<?php } ?>/>
		<input type="button" title="Ir a último" class="last" value="" <?php if($url_pages[4]){?> onclick="self.location='<?php echo $url_pages[4]; ?>';"<?php } ?>/>
	</div>
	<div class="clear"/></div>
</div>
